    ...
    root.innerHTML = `
        <div class="product">
            ...
            <div class="product-info">
                <h1>${p.name}</h1>
                <div class="meta-row">${p.gender} · ${p.type} · ${p.fit}</div>
                <div class="price-big">${formatPrice(p.price)}</div>

                <p class="desc">${escapeHtml(desc)} ${toggleBtn}</p>

                <ul class="specs">
                    <li><span>matière</span><span>${p.material}</span></li>
                    <li><span>coupe</span><span>${p.fit}</span></li>
                    <li><span>genre</span><span>${p.gender}</span></li>
                    <li><span>type</span><span>${p.type}</span></li>
                </ul>
